...
<section class="summary"><span id="shown">0 shown</span><span id="installed">0 installed</span><span id="total">0 total</span></section><main id="packages" class="grid"><div class="loading">Downloading ${noun} metadata…</div></main>
<script nonce="${nonce}">const vscode=acquireVsCodeApi();const root=document.getElementById('packages');const search=document.getElementById('search');const category=${isClick ? "document.getElementById('category')" : 'null'};let all=[];function esc(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));}function render(){const q=search.value.trim().toLowerCase();const cat=category?category.value:'';const items=all.filter(p=>(!cat||p.category===cat)&&(!q||[p.displayName,p.name,p.category].some(v=>String(v||'').toLowerCase().includes(q))));document.getElementById('shown').textContent=items.length+' shown';document.getElementById('installed').textContent=all.filter(p=>p.status==='installed').length+' installed';document.getElementById('total').textContent=all.length+' total';if(!items.length){root.innerHTML='<div class="empty">No examples match this filter.</div>';return;}root.innerHTML=items.map(p=>'<article class="card '+esc(p.status||'missing')+'"><div><h3>'+esc(p.displayName)+'</h3><div class="meta">'+(p.category?'<span>'+esc(p.category)+'</span>':'')+'<span>'+esc(p.status||'missing')+'</span></div>'+(p.status==='installed'?'<p><code>'+esc(p.root||'')+'</code></p>':'')+'</div><div class="actions">'+(p.status==='installed'?'<button class="secondary" data-open="'+esc(p.key)+'">Open Project</button><button class="danger" data-uninstall="'+esc(p.key)+'">Uninstall</button>':'<button data-install="'+esc(p.key)+'">Install</button>')+'</div></article>').join('');}root.onclick=e=>{const b=e.target.closest('button');if(!b)return;if(b.dataset.install)vscode.postMessage({type:'install',key:b.dataset.install});else if(b.dataset.uninstall)vscode.postMessage({type:'uninstall',key:b.dataset.uninstall});else if(b.dataset.open)vscode.postMessage({type:'openProject',key:b.dataset.open});};search.oninput=render;if(category)category.onchange=render;window.addEventListener('message',e=>{if(e.data?.type==='state'){all=e.data.items||[];if(category){const current=category.value;const categories=e.data.categories||[];category.innerHTML='<option value="">All categories</option>'+categories.map(c=>'<option value="'+esc(c)+'">'+esc(c)+'</option>').join('');category.value=categories.includes(current)?current:'';}render();}});vscode.postMessage({type:'ready'});</script></body></html>`;
}
