...
<section class="summary"><span id="shown">0 shown</span><button id="installed" class="filterChip" type="button" title="Show only installed examples" aria-pressed="false">0 installed</button><span id="total">0 total</span></section><main id="packages" class="grid"><div class="loading">Downloading Click Board metadata…</div></main>
<script nonce="${nonce}">const vscode=acquireVsCodeApi();const root=document.getElementById('packages');const search=document.getElementById('search');const category=document.getElementById('category');const installedChip=document.getElementById('installed');let all=[];let installedOnly=false;function esc(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));}function installed(p){return p.status==='installed'||p.status==='update';}function render(){const q=search.value.trim().toLowerCase();const cat=category.value;const items=all.filter(p=>(!installedOnly||installed(p))&&(!cat||p.category===cat)&&(!q||[p.displayName,p.category,p.name].some(v=>String(v||'').toLowerCase().includes(q))));document.getElementById('shown').textContent=items.length+' shown';const installedCount=all.filter(installed).length;installedChip.textContent=installedOnly?installedCount+' installed · showing only':installedCount+' installed';installedChip.classList.toggle('active',installedOnly);installedChip.setAttribute('aria-pressed',String(installedOnly));document.getElementById('total').textContent=all.length+' total';if(!items.length){root.innerHTML='<div class="empty">No Click Board examples match this filter.</div>';return;}root.innerHTML=items.map(p=>'<article class="card '+esc(p.status||'missing')+'"><div><h3>'+esc(p.displayName)+'</h3><div class="meta"><span>'+esc(p.category)+'</span><span>'+esc(p.status||'missing')+'</span></div>'+(installed(p)?'<p><code>'+esc(p.root||'')+'</code></p>':'')+'</div><div class="actions">'+(installed(p)?'<button class="secondary" data-open="'+esc(p.key)+'">Open Project</button><button class="danger" data-uninstall="'+esc(p.key)+'">Uninstall</button>':'<button data-install="'+esc(p.key)+'">Install</button>')+'</div></article>').join('');}function setCategories(categories){const current=category.value;category.innerHTML='<option value="">All categories</option>'+categories.map(c=>'<option value="'+esc(c)+'">'+esc(c)+'</option>').join('');category.value=categories.includes(current)?current:'';}root.onclick=e=>{const b=e.target.closest('button');if(!b)return;if(b.dataset.install)vscode.postMessage({type:'install',key:b.dataset.install});else if(b.dataset.uninstall)vscode.postMessage({type:'uninstall',key:b.dataset.uninstall});else if(b.dataset.open)vscode.postMessage({type:'openProject',key:b.dataset.open});};search.oninput=render;category.onchange=render;installedChip.addEventListener('click',()=>{installedOnly=!installedOnly;render();});window.addEventListener('message',e=>{if(e.data?.type==='state'){all=e.data.items||[];setCategories(e.data.categories||[]);render();}});vscode.postMessage({type:'ready'});</script></body></html>`;
}
...
<section class="summary"><span id="shown">0 shown</span><button id="installed" class="filterChip" type="button" title="Show only installed examples" aria-pressed="false">0 installed</button><span id="total">0 total</span></section><main id="packages" class="grid"><div class="loading">Downloading Demo metadata…</div></main>
<script nonce="${nonce}">const vscode=acquireVsCodeApi();const root=document.getElementById('packages');const search=document.getElementById('search');const installedChip=document.getElementById('installed');let all=[];let installedOnly=false;function esc(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));}function installed(p){return p.status==='installed'||p.status==='update';}function render(){const q=search.value.trim().toLowerCase();const items=all.filter(p=>(!installedOnly||installed(p))&&(!q||[p.displayName,p.name].some(v=>String(v||'').toLowerCase().includes(q))));document.getElementById('shown').textContent=items.length+' shown';const installedCount=all.filter(installed).length;installedChip.textContent=installedOnly?installedCount+' installed · showing only':installedCount+' installed';installedChip.classList.toggle('active',installedOnly);installedChip.setAttribute('aria-pressed',String(installedOnly));document.getElementById('total').textContent=all.length+' total';if(!items.length){root.innerHTML='<div class="empty">No Demo examples match this search.</div>';return;}root.innerHTML=items.map(p=>'<article class="card '+esc(p.status||'missing')+'"><div><h3>'+esc(p.displayName)+'</h3><div class="meta"><span>'+esc(p.status||'missing')+'</span></div>'+(installed(p)?'<p><code>'+esc(p.root||'')+'</code></p>':'')+'</div><div class="actions">'+(installed(p)?'<button class="secondary" data-open="'+esc(p.key)+'">Open Project</button><button class="danger" data-uninstall="'+esc(p.key)+'">Uninstall</button>':'<button data-install="'+esc(p.key)+'">Install</button>')+'</div></article>').join('');}root.onclick=e=>{const b=e.target.closest('button');if(!b)return;if(b.dataset.install)vscode.postMessage({type:'install',key:b.dataset.install});else if(b.dataset.uninstall)vscode.postMessage({type:'uninstall',key:b.dataset.uninstall});else if(b.dataset.open)vscode.postMessage({type:'openProject',key:b.dataset.open});};search.oninput=render;installedChip.addEventListener('click',()=>{installedOnly=!installedOnly;render();});window.addEventListener('message',e=>{if(e.data?.type==='state'){all=e.data.items||[];render();}});vscode.postMessage({type:'ready'});</script></body></html>`;
}
