...
class MCUConfigurator(QWidget):
    ...
    def save_parameters(self):
        ...
        cfg_target = f'{selected_mcu[:7].lower()}x.cfg'

        with open(os.path.join(os.getcwd(), 'runner', 'xpack-openocd-0.12.0-7', 'bin', cfg_target), 'w') as output_cfg:
            output_cfg.write(cfg_lines.replace('{target_name}', cfg_target))

        # After saving everything successfully
        QMessageBox.information(self, "Success", "System parameters saved successfully!")

        # Open the project window
        self.project_window = ProjectWindow(selected_mcu, cfg_target, target)
