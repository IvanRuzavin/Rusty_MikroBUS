...
use thermo19::{Thermo19, Thermo19Config};
...
fn main() -> ! {
    ...
    let config = Thermo19Config {
        gp0: mikrobus::MIKROBUS_1_PWM,
        gp1: mikrobus::MIKROBUS_1_AN,
        alt: mikrobus::MIKROBUS_1_INT,
        ..Default::default()
    };
    let mut thermo = Thermo19::new(config).unwrap_or_else(|_| loop {});
    if thermo.default_config().is_err() {
        if let Some(uart) = log_uart.as_mut() { let _ = uart_println(uart, " Default configuration error."); }
        loop {}
    }
    if let Some(uart) = log_uart.as_mut() { let _ = uart_println(uart, " Application Task "); }

    loop {
        if let Ok(temperature) = thermo.read_temperature() {
            if let Some(uart) = log_uart.as_mut() {
                let _ = uart_print(uart, " Temperature: ");
                uart_print_fixed(uart, temperature, 2);
                let _ = uart_println(uart, " C");
            }
        }
    }
}
